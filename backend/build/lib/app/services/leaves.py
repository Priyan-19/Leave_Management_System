...
class LeaveService:
    ...
    def _list_student_requests(self, roll_number: str) -> list[LeaveRecord]:
        query = self.leaves_collection.where(filter=FieldFilter("rollNo", "==", roll_number))
        requests = [self._doc_to_leave(doc) for doc in query.stream()]
        return sorted(requests, key=lambda leave: leave.created_at, reverse=True)
    ...
    def list_student_leaves(
        ...
        requests = self._list_student_requests(current_user.roll_number or "")
        if status_filter:
            requests = [leave for leave in requests if leave.status == status_filter]
        if start_date:
            requests = [leave for leave in requests if leave.start_date >= start_date]
        if end_date:
            requests = [leave for leave in requests if leave.end_date <= end_date]
        return requests
    ...
    def list_leave_requests(
        ...
        if search:
            ...
            requests = [
                ...
                if needle in leave.student_name.lower() or needle in leave.roll_number.lower()
            ]
        if start_date:
            requests = [leave for leave in requests if leave.start_date >= start_date]
        if end_date:
    ...
    def get_student_dashboard(self, current_user: UserProfile) -> StudentDashboardSummary:
        student = self.user_service.get_student_by_roll_number(current_user.roll_number or "")
        all_requests = self._list_student_requests(student.roll_number)
