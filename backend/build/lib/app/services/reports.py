...
class ReportService:
    ...
    def _student_count(self, current_user: UserProfile) -> int:
        if current_user.role == UserRole.STAFF:
            query = self.students_collection.where(filter=FieldFilter("staffId", "==", current_user.staff_id))
        else:
    ...
    def _staff_count(self) -> int:
        return sum(
            ...
            if doc.to_dict().get("role") == UserRole.STAFF.value and doc.to_dict().get("isActive", True)
        )

    def _scoped_leaves(self, current_user: UserProfile) -> list[LeaveRecord]:
        return self.leave_service.list_leave_requests(current_user)
    ...
    def _build_dashboard_payload(self, current_user: UserProfile) -> dict:
        leaves = sorted(self._scoped_leaves(current_user), key=lambda leave: leave.created_at, reverse=True)
    ...
    def build_report(
        ...
        scope = "Global institution report" if current_user.role == UserRole.SUPER_ADMIN else "Staff-scoped report"
        leaves = [
            leave
            for leave in self._scoped_leaves(current_user)
