...
import { BarChart3, CheckCircle2, ClipboardList, Clock3, Users } from 'lucide-react'
...
export function StaffDashboardPage() {
  ...
  if (isLoading) {
    return <PanelLoader label="Loading staff analytics..." />
  }

  if (isError || !data) {
    return (
      <EmptyState
        description="The staff dashboard could not be loaded right now. Please try again shortly."
  ...
  return (
    <div className="space-y-6">
      <SectionHeading
        description="Monitor leave activity for your assigned students, stay ahead of pending approvals, and spot the busiest learners at a glance."
        title="Staff dashboard"
      />

      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard detail="Students assigned to your scope." icon={Users} label="Managed students" value={data.total_students} />
        <StatCard detail="Leave requests submitted by your students." icon={ClipboardList} label="Total leaves" value={data.total_requests} />
        <StatCard detail="Requests already approved." icon={CheckCircle2} label="Approved" value={data.approved_count} />
        <StatCard detail="Requests still waiting for your decision." icon={Clock3} label="Pending" value={data.pending_count} />
      </section>
      ...
      <article className="data-panel">
        ...
        {data.recent_requests.length === 0 ? (
          <div className="p-6">
            <EmptyState
              description="Once your students start applying for leave, the latest requests will appear here."
